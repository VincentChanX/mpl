...
dnl PHP_ARG_WITH(mpl, for mpl support,
dnl Make sure that the comment is aligned:
dnl [  --with-mpl             Include mpl support])

dnl Otherwise use enable:

PHP_ARG_ENABLE(mpl, whether to enable mpl support,
Make sure that the comment is aligned:
[  --enable-mpl           Enable mpl support])
...
if test "$PHP_MPL" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $MPL_DIR/$PHP_LIBDIR, MPL_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_MPLLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong mpl lib version or lib not found])
  dnl ],[
  dnl   -L$MPL_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
  dnl PHP_SUBST(MPL_SHARED_LIBADD)

  PHP_NEW_EXTENSION(mpl, mpl.c library.c, $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
